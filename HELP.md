...
**Chat logs** - optional and **off by default**; the first login asks once. When on, IMs are kept as plain text files on this device, split per account - one file per person, named by their **username** (`alice.wonder.txt`, never a display name), under `<account-id>/logs/avatars/`, one per group or conference under `<account-id>/logs/groups/`, in the app's data folder (never the OS cache). People, groups and **nearby chat** (`<account-id>/logs/local/`) have **separate switches** in **Bee -> Settings**. Files from before the per-account split stay where they were (a shared `logs/` folder) and show up in the manager as **Older shared logs**. **Bee -> About** shows how much space the logs take and the folder they live in - the figure is a link into the **log manager**, where single conversations or whole categories can be deleted (with a confirmation; deletion is permanent). When a conversation that has a log file opens - by hand or because a message came in - the last ten logged lines appear dimmed at the top as history, above a "from your chat log" divider, so a new message lands with its context.
