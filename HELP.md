...
**Chat logs** - optional and **off by default**; the first login asks once. When on, IMs are kept as plain text files on this device, split per account - one file per person under `<account-id>/logs/avatars/`, one per group or conference under `<account-id>/logs/groups/`, in the app's data folder (never the OS cache). People, groups and **nearby chat** (`<account-id>/logs/local/`) have **separate switches** in **Bee -> Settings**. Files from before the per-account split stay where they were (a shared `logs/` folder) and show up in the manager as **Older shared logs**. **Bee -> About** shows how much space the logs take and the folder they live in - the figure is a link into the **log manager**, where single conversations or whole categories can be deleted (with a confirmation; deletion is permanent).

**Settings on disk** - preferences, the remembered username/grid, and MFA "remember device" tokens live in `settings.json` in the same data folder, so they survive a webview data wipe and are shared by every viewer window. Running **more than one viewer at once** (say, two accounts) works; they share that settings file, and each account's chat logs land in its own folder.
