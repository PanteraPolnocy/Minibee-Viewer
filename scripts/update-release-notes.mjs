#!/usr/bin/env node
/**
 * Patches a GitHub release body: keeps the author's notes, inserts a per-platform
 * download table (with checksums), then appends the README/LICENSE footer.
 *
 * Usage:
 *   node scripts/update-release-notes.mjs --release-id 12345
 *   node scripts/update-release-notes.mjs --tag 0.8.1 --dry-run
 */
...
`);
  ...
  return { releaseId, tag, repo, dryRun };
}
 ...
 * @param {string} repo
 */
export function composeReleaseBody(release, repo) {
  ...
  const parts = [userNotes, downloads, footer].filter((part) => part.length > 0);
  return parts.join('\n\n');
}
...
async function main() {
  const { releaseId, tag, repo, dryRun } = parseArgs(process.argv);
  const release = await fetchRelease(repo, { releaseId, tag });
  const body = composeReleaseBody(release, repo);
