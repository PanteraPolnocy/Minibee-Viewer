...
const BeeBridge = (function () {
  ...
  function listen(event: string, handler: (payload: any) => void): Promise<() => void> {
    const t = tauri();
  ...
  Bridge.prototype._proxyContext = function (options): ProxyContext {
  ...
  Bridge.prototype.slSendRaw = function (sessionId, packetB64, simIp, simPort) {
    const args: { sessionId: string; packet: string; simIp?: string; simPort?: number } =
      { sessionId: sessionId, packet: packetB64 };
